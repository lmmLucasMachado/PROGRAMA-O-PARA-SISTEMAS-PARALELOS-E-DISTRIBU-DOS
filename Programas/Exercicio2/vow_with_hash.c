...
rand_itpset(IT_POINT_SET *itpset, POINT_T *Psums, POINT_T *Qsums,
    ...
    int pedaco = L/NUM_THREADS;
    int start = thread_num*pedaco;
    int fim = (thread_num+1)*pedaco;
    ...
    MPI_Gather(&itPsetBase.itpoint[start], pedaco, point_struct, &itPsetBase.itpoint[resto], pedaco, point_struct, 0, MPI_COMM_WORLD);
    MPI_Bcast(&itPsetBase, 1, it_point_struct, 0, MPI_COMM_WORLD);
